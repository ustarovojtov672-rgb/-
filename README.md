...
GET `/api/analyze-meal` возвращает диагностику агента: активный runtime, модель, наличие Pi auth, nutrition skill, codex-search skill и project-local пакета `pi-codex-search`. Этот же статус показывается в блоке `AI-агент` внутри приложения.
