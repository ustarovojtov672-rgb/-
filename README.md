...
Серверный слой приложения уже выделен в `src/lib/nutrition-agent/`: туда приходит текст, фото, профиль, цель, дневные ориентиры, подтвержденная память блюд и последние приемы пищи. `tool-plan.ts` собирает план инструментов для агента: текст, зрение, OCR, штрихкод, память, локальная база и web search. `pi-agent.ts` подключает Pi SDK к `/api/analyze-meal`, передает фото как image input и использует локальное хранилище авторизации из `Аi агент питания/.pi-agent/`. UI показывает черновик расчета, использованные инструменты, найденную еду, источники, похожие подтвержденные блюда и предупреждение, если порцию нужно проверить вручную. После сохранения проверенная еда попадает в Jazz-память аккаунта и используется в следующих анализах.
